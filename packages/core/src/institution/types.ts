 ...
 *
 * Analysis is evidence-based: every OpenAlex article-type DOI is probed
 * directly against Crossref and grouped by the member + publisher Crossref
 * itself reports. There is no hand-maintained OpenAlex→Crossref map.
 * See `analyze.ts`. Consumed by the web app and the MCP server via
 * `@nexus-score/core`.
 */
